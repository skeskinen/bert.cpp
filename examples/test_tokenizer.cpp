...
void tokenizer_test(const bert_vocab& vocab, const std::string& input, const std::vector<bert_vocab::id>& expected) {
    auto result = ::bert_tokenize(vocab, input);
    if (result != expected) {
        ...
        for (size_t i = 0; i < result.size(); i++) {
            bert_vocab::id a = expected[std::min(i, expected.size()-1)];
            bert_vocab::id b = result[i];
            const char *color_start = (a == b) ? ANSI_COLOR_GREEN : ANSI_COLOR_RED;
            const char *color_end = ANSI_COLOR_RESET;

            printf("%s%d -> %s : %d -> %s%s\n", color_start, a, vocab.id_to_token(a).data(), b, vocab.id_to_token(b).data(), color_end);
        }
    } else {
        printf("Success '%.*s...'\n", 16, input.data());
    }
